...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "IoP.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "IoPd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong IoP will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.IoP.it/wiki/BIP_0014)
