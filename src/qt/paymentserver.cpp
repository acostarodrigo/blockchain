...
const char* IoP_REQUEST_MIMETYPE = "application/IoP-paymentrequest";
const char* IoP_PAYMENTACK_MIMETYPE = "application/IoP-paymentack";
const char* IoP_PAYMENTACK_CONTENTTYPE = "application/IoP-payment";
...
//
// OSX-specific way of handling IoP: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on IoP: URIs creates FileOpen events on the Mac
    ...
    if (s.startsWith(IoP_IPC_PREFIX, Qt::CaseInsensitive)) // IoP: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseIoPURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid IoP address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom IoP addresses are not supported
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", IoP_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, IoP_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", IoP_PAYMENTACK_MIMETYPE);
