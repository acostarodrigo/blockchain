...
*Setup instructions for a gitian build of IoP using a Debian VM or physical system.*
...
In [doc/release-process.md](release-process.md) in the IoP repository under 'Fetch and build inputs'.
...
To build IoP (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the IoP repository.
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/IoP-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/IoP-build.assert
```
...
[IoP/gitian.sigs](https://github.com/IoP/gitian.sigs/) repository, or
